...
Ras2Pt <- function(r=r, prod='RADD'){
  ...
  #just for projection to UTM Zone 51N
...
### 4. Create nice cumulative graphs of the alerts (Figure 4 of paper)
setwd('C:/defoPH/data')
...
### 5. Create point data of quarterly deforestation pixels from 2000-2020 separately for RADD and GLAD
...
Quarterly <- function(prod='GLAD'){
  ...
  fl.alrt <- rbind(r.pts[,c('date', 'qrtr')], r.pts1[,c('date', 'qrtr')])
...
### 6. Quarterly graph of deforestation pixels inside and outside hotspots (Figure 5)
...
eha.qrtr <- lapply(eha.all1, function(x) subset(x, x$PATTERN != 'No Pattern Detected'))
...
######## change..
p1 = ggplot(data=d1,aes(x=qrtr,y=ha1, color=prod)) +scale_color_manual(values=c("dark green", "dark blue")) +
  geom_line(stat='identity',position=position_dodge())+geom_point(size=2)+ theme_bw() + xlab("Quarter, 2020")+
  ylab("Deforested area (ha)") +   ylim(0, 1800)+ ggtitle("New")+
  theme(text = element_text(size=18), panel.grid.major = element_blank(),
        plot.title = element_text(size=18,vjust = - 10,hjust = +.9),
        panel.grid.minor = element_blank(), legend.position='none')
p2 = ggplot(data=d2,aes(x=qrtr,y=ha1, color=prod)) +scale_color_manual(values=c("dark green", "dark blue")) +
  geom_line(stat='identity',position=position_dodge())+geom_point(size=2)+ theme_bw() + xlab("Quarter, 2020")+
  ylab("Deforested area (ha)") +   ylim(0, 1800)+ ggtitle("Consecutive")+
...
p3 = ggplot(data=d3,aes(x=qrtr,y=ha1, color=prod)) +scale_color_manual(values=c("dark green", "dark blue")) +
  geom_line(stat='identity',position=position_dodge())+geom_point(size=2)+ theme_bw() + xlab("Quarter, 2020")+
  ylab("Deforested area (ha)") +   ylim(0, 1800)+ ggtitle("Sporadic")+
  theme(text = element_text(size=18), panel.grid.major = element_blank(), 
        plot.title = element_text(size=18,vjust = - 10,hjust = +.9),
        panel.grid.minor = element_blank(), legend.position='none')
p4 = ggplot(data=d4,aes(x=qrtr,y=ha1, color=prod)) +scale_color_manual(values=c("dark green", "dark blue")) +
  geom_line(stat='identity',position=position_dodge())+geom_point(size=2)+ theme_bw() + xlab("Quarter, 2020")+
  ylab("Deforested area (ha)") +   ylim(0, 1800)+ ggtitle("Outside")+
...
ggsave(p, file=paste0("Figure4a.png"), width = 12, height = 10, units = "in")

d1$PATTERN <- 'New'
d2$PATTERN <- 'Consecutive'
d3$PATTERN <- 'Sporadic'
d4$PATTERN <- 'Outside'
write.csv(rbind(d1,d2,d3,d4), 'eha.all1.csv',row.names=F)
    
  
    out.defo.glad <-  lapply(eha.defo.glad, function(x) subset(x, is.na(x$PATTERN)))
    out.defo.radd <- lapply(eha.defo.radd, function(x) subset(x, is.na(x$PATTERN)))
    out.defo.glad <- lapply(out.defo.glad, function(x) {x$n <- 1;return(x)})
    out.defo.radd <- lapply(out.defo.radd, function(x) {x$n <- 1;return(x)})
    gl <- c(sum(out.defo.glad[[1]]$n),sum(out.defo.glad[[2]]$n),sum(out.defo.glad[[3]]$n),
            sum(out.defo.glad[[4]]$n))
    ra <- c(sum(out.defo.radd[[1]]$n),sum(out.defo.radd[[2]]$n),sum(out.defo.radd[[3]]$n),
            sum(out.defo.radd[[4]]$n))
    out.glad <- data.frame(c(1:4),gl, 'GLAD')
    out.radd <-data.frame(c(1:4),ra, 'RADD')
    names(out.glad) <- names(out.radd)
    d.outside <- rbind(out.glad,out.radd)
    d.outside$defo <- 'Outside'
    
    d1 <- ddply(as.data.frame(aa), .(qrtr, PATTERN), summarize, sum=sum(defo))
    d2 <- ddply(as.data.frame(bb), .(qrtr), summarize, sum=sum(defo))
    
    d1$sum1 <- cumsum(d1$sum)
    d2$sum1 <- cumsum(d2$sum)
    d1$prod <- 'RADD'
    d2$prod <- 'GLAD'
    d3 <- rbind(d1,d2)
    d3$defo <- 'Outside'
    d3 <- subset(d3, !is.na(d3$qrtr))
    d3 <- d3[,-2]
    d3 <- d3[with(d3, order(prod)), ]
    
    names(d.inside) <- names(d3)
    d3$sum1 <- d3$sum1 - d.inside$sum1 #net outside
    d.in.out <- rbind(d3,d.inside)
    d.in.out$ha <-  (d.in.out$sum1 * 900) / 10000
    
    # hotspot summary
    eha.all2 <- ldply(lapply(eha.all1, function(x)  
      aggregate(n ~ PATTERN, FUN = sum, data = x, na.rm = TRUE)),data.frame)
    
    eha.all2$qrtr <-  c(rep(1,6),rep(2,6),rep(3,6),rep(4,6)
                        ,rep(1,6),rep(2,6),rep(3,6),rep(4,6))
    eha.all2$product <-  c(rep('GLAD',24),rep('RADD',24))
    eha.all2$defo <- 'Hotspot'
    eha.all2 <- subset(eha.all2, eha.all2$PATTERN=='New Hot Spot')
    
    # all together 
    eha <- data.frame(eha.all2$qrtr,eha.all2$n, eha.all2$product,'new_hotspot' ,eha.all2$n * 100)
    names(eha) = names(d.in.out)
    eha.all <- rbind(eha, d.in.out)
    

### tally per hotspot class per alert product
setwd('C:/defoPH/intermediate')
glad.pt <- readOGR(getwd(), 'eha_glad_fl_4yr')
glad.pt$pt <- gCentroid(glad_4, byid=TRUE)
...
radd.pt$pt <- gCentroid(radd_4, byid=TRUE)
radd.pt$n <- 1
...
p4
setwd('C:/defoPH/results')
ggsave(p4, file=paste0("Fig3.png"), width = 8, height = 4, units = "in")
